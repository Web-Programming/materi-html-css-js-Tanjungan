...
// 2. fungsi untuk merender produk ke dalam DOM
function renderProducts(productList) {
    //DOM : menyeleksi elemen container
    const container = document.getElementById("product-container");
    container.innerHTML = ""; // Clear container terlebih dahulu
    ...
    productList.forEach(product => {
        ...
        if (product.diskon > 0) {
            ...
            text-decoration-line-through small">Rp ${product.diskon.toLocaleString("id-ID")}</span><br>
            <span class="fw-bold text-danger">Rp ${product.harga.toLocaleString("id-ID")}</span>`; 
        } else {
            priceHtml = `<span class="fw-bold text-dark">Rp ${product.harga.toLocaleString("id-ID")}</span>`;
        }
        //mengecek stok produk
        if (product.stok > 0) {
            buttonHtml = `<button class="btn btn-buy p-2" onclick="alert('Ditambahkan ke keranjang: ${product.nama}')">
        ...
        <div class="col-12 col-md-6 col-lg-4 col-xl-3">
            ...
            ${badgeHtml}
                ...
                <div class="card-body">
                    <small class="text-uppercase text-muted" style="font-size: 0.7em;">${product.kategori}</small>
                    <h6 class="card-title fw-bold mb-3 mt-1">${product.nama}</h6>
                    <p class="card-text mb-2">${priceHtml}</p>
                    ${buttonHtml}
                </div>
            </div>
        </div>
        `;
        // Meniysipkan HTML ke dalam container
